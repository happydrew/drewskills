...
- wrapper 先检查 `9225`，未监听时自动尝试补起本地 Edge/Chrome CDP 会话
- 仍然保持边界：技能不负责安装浏览器或处理代理，只在既有本机浏览器上补起本地 CDP 会话
...
- `scripts/run_trends_collect.ps1` 现会在执行前检查并按需补起本地 `127.0.0.1:9225` CDP 会话
...
- 如果后续需要自动化浏览器安装、代理切换或更复杂的多会话管理，仍应拆到独立技能，不要把本技能继续扩成环境管理器
